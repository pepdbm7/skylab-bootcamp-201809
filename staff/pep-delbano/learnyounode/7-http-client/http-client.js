...
http.get(process.argv[2], function (response) {  //pasamos al método http.get una URL, y una callback
  response.setEncoding('utf8')  //es un método de http.get(), y con el parámetro utf8, sería lo mismo que hacer toString() a los datos obtenidos (el response), necesario para imprimirlo por consola
  response.on('data', console.log)  //imprimimos esos datos obtenidos
  response.on('error', console.error) //imprimimos error si hubiera (obtenemos un error del server)
}).on('error', console.error) //imprimimos error (si faltaran parámetros y no pudieramos hacer la peticion, x ej)
